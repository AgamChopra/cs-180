...
public class Preprocessing {
	...
	public static void main(String[] args) {
        ...
        System.out.print("Enter the bin range: ");
        int b = scan.nextInt();
        double jump = 1.0 / b;
        ...
        for(int j = 0; j < 5; j++){
            for (int i = 0; i < dataSet.length; i++) {
               while(flag){
                   ...
                   if(dataSet[i][j] - 0.00001 <= (jump + (jump * n))) {
                       dataSet[i][j] = (double)n;
                       flag = false;
                   }
                   else
                       n++;
        ...
        int ascii_sum = 0;
        int[] label_encoded = new int[degreeColumn.length];
        for(int i = 0; i < degreeColumn.length; i++){
            degreeColumn[i] = degreeColumn[i].toLowerCase();
            for(int j = 0; j < degreeColumn[i].length(); j++){
                ascii_sum = ascii_sum + (int)degreeColumn[i].charAt(j);
            }
            label_encoded[i] = ascii_sum;
            ascii_sum = 0;
        }
        ...
        int[][] answer = new int[dataSet.length][(dataSet[0].length)+1];
        for(int i = 0; i < dataSet.length; i++){
            answer[i][0] = label_encoded[i];
            for(int j = 0; j < dataSet[0].length; j++) {
                answer[i][j + 1] = (int)Math.round(dataSet[i][j]);
            }
        }

        doMachineLearning(answer);
